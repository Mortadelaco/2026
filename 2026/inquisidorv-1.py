import json
import os
from datetime import datetime, date

# --- CONFIGURACIÓN ---
DB_FILE = "inquisidor_db.json"
VERSION = "1.2"


class Inquisidor:
    def __init__(self):
        self.data = self.load_data()

    # ------------------------------------------------------------------ #
    #  PERSISTENCIA                                                        #
    # ------------------------------------------------------------------ #

    def load_data(self):
        if not os.path.exists(DB_FILE):
            return []
        try:
            with open(DB_FILE, 'r', encoding='utf-8') as f:
                return json.load(f)
        except (json.JSONDecodeError, ValueError):
            print("⚠️  El archivo de datos estaba corrupto. Se ha iniciado con una base de datos vacía.")
            return []

    def save_data(self):
        """Guarda los datos con manejo de errores de I/O."""  # CORRECCIÓN #6
        try:
            with open(DB_FILE, 'w', encoding='utf-8') as f:
                json.dump(self.data, f, indent=4, default=str)
        except OSError as e:
            print(f"❌ Error al guardar los datos: {e}")
            print("   Comprueba los permisos del directorio o el espacio en disco.")

    # ------------------------------------------------------------------ #
    #  HELPERS DE VALIDACIÓN                                               #
    # ------------------------------------------------------------------ #

    @staticmethod
    def _pedir_float(mensaje, min_val, max_val):
        """Pide un float en [min_val, max_val] hasta que el usuario introduzca uno válido."""  # CORRECCIÓN #2
        while True:
            try:
                valor = float(input(mensaje))
                if min_val <= valor <= max_val:
                    return valor
                print(f"   Por favor, introduce un número entre {min_val} y {max_val}.")
            except ValueError:
                print("   Error: introduce un número válido.")

    @staticmethod
    def _pedir_int_no_negativo(mensaje):
        """Pide un entero >= 0."""  # CORRECCIÓN #2
        while True:
            try:
                valor = int(input(mensaje))
                if valor >= 0:
                    return valor
                print("   El valor no puede ser negativo.")
            except ValueError:
                print("   Error: introduce un número entero válido.")

    @staticmethod
    def _confirmar(mensaje):
        """Pregunta s/n y acepta variantes comunes de 'sí'."""  # CORRECCIÓN #8
        while True:
            resp = input(mensaje).strip().lower()
            if resp in ('s', 'si', 'sí', 'yes', 'y'):
                return True
            if resp in ('n', 'no'):
                return False
            print("   Responde con 's' o 'n'.")

    # ------------------------------------------------------------------ #
    #  LÓGICA PRINCIPAL                                                    #
    # ------------------------------------------------------------------ #

    def registrar_dia(self, fecha_str=None, es_backup=False):
        """Registra los datos del día. Si fecha_str es None, usa hoy."""
        if not fecha_str:
            fecha_str = str(date.today())
            print(f"\n--- REGISTRO DIARIO ({fecha_str}) ---")
        else:
            print(f"\n--- BACKUP MODE ({fecha_str}) ---")

        # Verificar si ya existe un registro para esa fecha
        for entry in self.data:
            if entry['fecha'] == fecha_str:
                print("⚠️  Ya existe un registro para esta fecha.")
                if not self._confirmar("¿Sobreescribir? (s/n): "):  # CORRECCIÓN #8
                    return
                self.data.remove(entry)
                break

        friccion  = self._pedir_float("1. Nivel de Fricción (0-3): ", 0, 3)          # CORRECCIÓN #2
        prod_base = self._pedir_float("2. % Realismo Base percibido (0-100): ", 0, 100)  # CORRECCIÓN #2

        print("\n3. Autoobservación (Mínimo 15 min de percepción pura)")
        auto_observacion = self._confirmar("¿Has reflexionado objetivamente hoy? (s/n): ")  # CORRECCIÓN #8

        idealismos = self._pedir_int_no_negativo("4. ¿Cuántos idealismos detectaste hoy?: ")  # CORRECCIÓN #2

        nuevo_registro = {
            "fecha": fecha_str,
            "friccion": friccion,
            "realismo_base": prod_base,
            "auto_observacion": auto_observacion,
            "idealismos": idealismos,
            "es_backup": es_backup
        }

        self.data.append(nuevo_registro)
        self.data.sort(key=lambda x: x['fecha'])  # Mantener orden cronológico
        self.save_data()
        print("✅ Datos guardados.")

    def registrar_backup(self):
        print("\n--- MODO BACKUP ---")
        fecha_input = input("Introduce la fecha a recuperar (YYYY-MM-DD): ").strip()
        try:
            fecha_dt = datetime.strptime(fecha_input, "%Y-%m-%d").date()
        except ValueError:
            print("❌ Formato de fecha incorrecto. Usa YYYY-MM-DD.")
            return

        # CORRECCIÓN #5: no permitir fechas futuras
        if fecha_dt > date.today():
            print("❌ No puedes registrar datos para una fecha futura.")
            return

        self.registrar_dia(fecha_str=fecha_input, es_backup=True)

    def calcular_estadisticas(self):
        if not self.data:
            print("No hay datos registrados.")
            return

        mes_actual = str(date.today())[:7]  # YYYY-MM
        registros_mes = [r for r in self.data if r['fecha'].startswith(mes_actual)]

        if not registros_mes:
            print(f"No hay datos para {mes_actual}.")
            return

        total_dias = len(registros_mes)
        media_base = sum(r['realismo_base'] for r in registros_mes) / total_dias

        # Penalización por Idealismos
        total_idealismos   = sum(r.get('idealismos', 0) for r in registros_mes)
        penalizacion_idealismos = total_idealismos * 1.0

        # Factor Disciplina (Autoobservación: objetivo 2 veces por semana)
        # CORRECCIÓN #3: usar los días realmente registrados en el mes,
        # no el día-del-mes del sistema, para un cálculo más justo.
        dias_registrados      = total_dias
        semanas_transcurridas = max(1, dias_registrados / 7)
        objetivo_ao           = semanas_transcurridas * 2
        total_ao              = sum(1 for r in registros_mes if r['auto_observacion'])

        factor_disciplina = 1.0
        msg_disciplina    = "CUMPLIDO"
        if total_ao < objetivo_ao:
            factor_disciplina = 0.75
            msg_disciplina    = f"FALLIDO ({total_ao}/{int(objetivo_ao)})"

        # Fórmula final
        realismo_final = (media_base * factor_disciplina) - penalizacion_idealismos

        # CORRECCIÓN #4: avisar si el valor está fuera del rango lógico
        aviso_rango = ""
        if realismo_final < 0:
            aviso_rango = "  ⚠️  Valor por debajo de 0% (fuera del rango lógico)"
        elif realismo_final > 100:
            aviso_rango = "  ⚠️  Valor por encima de 100% (fuera del rango lógico)"

        print(f"\n=== INFORME MENSUAL ({mes_actual}) ===")
        print(f"📅 Días registrados:          {total_dias}")
        print(f"🧠 Realismo Base Promedio:    {media_base:.2f}%")
        print(f"👁️  Autoobservación:           {msg_disciplina} -> Factor {factor_disciplina}")
        print(f"🦄 Idealismos Totales:         {total_idealismos} -> Penalización -{penalizacion_idealismos:.2f}%")
        print("-" * 40)
        print(f"📊 REALISMO TOTAL: {realismo_final:.2f}%{aviso_rango}")

    def idealismo_rapido(self):
        """Suma 1 idealismo al día de hoy sin pedir el resto de datos."""
        hoy = str(date.today())
        encontrado = False

        for r in self.data:
            if r['fecha'] == hoy:
                r['idealismos'] = r.get('idealismos', 0) + 1
                encontrado = True
                print(f"🦄 Idealismo añadido. Total hoy: {r['idealismos']}")
                break

        if not encontrado:
            print("⚠️  No has iniciado el día. Creando registro básico...")
            nuevo = {
                "fecha": hoy,
                "friccion": 0,
                "realismo_base": 100,
                "auto_observacion": False,
                "idealismos": 1,
                "es_backup": False
            }
            self.data.append(nuevo)
            self.data.sort(key=lambda x: x['fecha'])  # CORRECCIÓN #1
            print("Registro creado con 1 idealismo. Recuerda rellenar el resto luego.")

        self.save_data()

    # ------------------------------------------------------------------ #
    #  MENÚ                                                                #
    # ------------------------------------------------------------------ #

    def menu(self):
        opciones = {'1', '2', '3', '4', '5'}
        while True:
            print(f"\n--- INQUISIDOR v{VERSION} ---")
            print("1. Registrar Hoy")
            print("2. Registrar Idealismo Rápido (Hoy)")
            print("3. Modo Backup (Registrar pasado)")
            print("4. Ver Estadísticas Mensuales")
            print("5. Salir")
            op = input("Elige: ").strip()

            # CORRECCIÓN #7: validar entradas del menú
            if op not in opciones:
                print("❌ Opción no válida. Elige un número del 1 al 5.")
                continue

            if   op == '1': self.registrar_dia()
            elif op == '2': self.idealismo_rapido()
            elif op == '3': self.registrar_backup()
            elif op == '4': self.calcular_estadisticas()
            elif op == '5':
                print("👋 Hasta luego.")
                break


# ------------------------------------------------------------------ #
#  PUNTO DE ENTRADA                                                    #
# ------------------------------------------------------------------ #

if __name__ == "__main__":
    app = Inquisidor()
    app.menu()
